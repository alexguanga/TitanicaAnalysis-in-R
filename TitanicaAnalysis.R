...
ggplot(data.combined[1:891, ], aes(x=Embarked, fill=factor(Survived))) +
  ...
  ylim(0,300) +
  labs(fill="Survived")
# Result: Not much info can be given by embarked


# Summary: Pclass, Title (combines age and sex into this), maybe we'll use sibsp and parch
# Familysize, 
...
randomforest.train.1 <- data.combined[1:891, c("Pclass", "Title")] # Only need the Pclass and Title columns
...
# ntree refers to the total amnt of trees you want to be used
randomforest.1 <- randomForest(x=randomforest.train.1, y=randomforest.label, importance=TRUE, ntree=1000)
randomforest.1
...
# and 168 would survive (~50 $ accurate)

varImpPlot(randomforest.1) 
# In this graph, more to the right mean they are more important



# Train a Random Forest with the parameters Pclass, title, and sibsb
randomforest.train.2 <- data.combined[1:891, c("Pclass", "Title", "SibSp")]

set.seed(1234)

randomforest.2 <- randomForest(x=randomforest.train.2, y=randomforest.label, importance=TRUE, ntree=1000)
randomforest.2
varImpPlot(randomforest.2)

# NOTE: OOB estimation rate decrease, the lower it is, the better it is. (lower the error rate)



# Train a Random Forest with the parameters Pclass, title, and Parch
randomforest.train.3 <- data.combined[1:891, c("Pclass", "Title", "Parch")]

set.seed(1234)

randomforest.3 <- randomForest(x=randomforest.train.3, y=randomforest.label, importance=TRUE, ntree=1000)
randomforest.3
varImpPlot(randomforest.3)



# Train a Random Forest with the parameters Pclass, title, and SibSp, Parch
randomforest.train.4 <- data.combined[1:891, c("Pclass", "Title", "SibSp", "Parch")]

set.seed(1234)

randomforest.4 <- randomForest(x=randomforest.train.4, y=randomforest.label, importance=TRUE, ntree=1000)
randomforest.4
varImpPlot(randomforest.4)



# Train a Random Forest with the parameters Pclass, title, and FamilySize
randomforest.train.5 <- data.combined[1:891, c("Pclass", "Title", "FamilySize")]

set.seed(1234)

randomforest.5 <- randomForest(x=randomforest.train.5, y=randomforest.label, importance=TRUE, ntree=1000)
randomforest.5
varImpPlot(randomforest.5)
...
randomforest.train.6 <- data.combined[1:891, c("Pclass", "Title", "SibSp", "FamilySize")]

set.seed(1234)

randomforest.6 <- randomForest(x=randomforest.train.6, y=randomforest.label, importance=TRUE, ntree=1000)
randomforest.6
varImpPlot(randomforest.6)
...
randomforest.train.7 <- data.combined[1:891, c("Pclass", "Title", "Parch", "FamilySize")]

set.seed(1234)

randomforest.7 <- randomForest(x=randomforest.train.7, y=randomforest.label, importance=TRUE, ntree=1000)
randomforest.7
varImpPlot(randomforest.7)
